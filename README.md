...
- maxr<br>
    ...
    At 1 TTempSmooth will be (at max) including pixels from 1 frame away in the average (3 frames total will be considered counting
    the current frame).<br>
    At 7 it would be including pixels from up to 7 frames away (15 frames total will be considered).<br>
    With the way it checks motion there isn't much danger in setting this high (within the mode's limits), it's basically a quality
    vs. speed option. Lower settings are faster while larger values tend to create a more stable image.<br>
    Default: 3.

- ythresh<br>
    Luma threshold. Its application depends on `pmode`:
    - For `pmode=0` and `pmode=2`: It's a threshold for differences of pixels between frames. TTempSmooth checks 2 frame distance as well
    as single frame, so these can usually be set slightly higher than with most other temporal smoothers and still avoid artifacts. Also
    important is the fact that as long as `ymdiff` is less than `ythresh`, pixels with larger differences from the original will have
    less weight in the average.
    - For `pmode=1`: After the "best" sample is selected (from temporal neighbors or IIR memory), this threshold is used to compare
    the selected "best" sample with the original pixel in the current frame. If the difference is less than `ythresh`, the "best" sample
    is output; otherwise, the original pixel from the current frame is output.
...
- ymdiff (only for `pmode=0` and `pmode=2`)<br>
    ...
    mdiff makes TTempSmooth treat pixels that have a difference of less than or equal to mdiff as though they have a difference of 0.
    In other words, it shifts the zero difference point outwards.<br>
...
- strength (only for `pmode=0` and `pmode=2`)<br>
    ...
    The strength option lets you shift the drop off point away from the center to give a stronger smoothing effect and add weight to
    the outer pixels.<br>
    ...
    The values shown are for maxr=7, when using smaller radius values the weights outside of the range are simply dropped. Thus, setting
    strength to a value of maxr+1 or higher will give you equal spatial weighting of all pixels in the kernel.<br>
    - For `pmode=2`:
    - `strength = 1`: Pure binomial weighting is applied across all `2*maxr + 1` frames. The weights are derived from Pascal's triangle for
    `k = 2*maxr`. This gives a classic bell-curve shape.
    - `strength > 1`: A plateau is formed. Frames at distance `d < strength` (i.e., the center frame and `strength-1` frames on each side)
    share a common peak weight. The falloff from the edges of this plateau then follows a binomial curve shape. The peak weight of
    the plateau is determined by the peak of the binomial curve used for the falloff.
...
- fp (only for `pmode=0` and `pmode=2`)<br>
    When true will add any weight not given to the outer pixels back onto the center pixel when computing the final value and it's much
    better for reducing artifacts in motion areas and usually produces overall better results. When false will just do a normal weighted
    average.<br>
    Default: True.
...
- pfclip<br>
    ...
    Basically, the pfclip will be used to determine the weights in the average but the weights will be applied to the original
    input clip's pixel values.
...
- pmode<br>
    ...
    1: It tries to select "the best" sample from the total pool of input samples in the temporal axis for current position in frame using
    an algorithm of selecting "most equal/probable" value.
    More info [here](https://github.com/Asd-g/AviSynth-vsTTempSmooth/pull/8#issuecomment-1616112102).<br>
    `pmode=1` with thresholds = 255 is more or less equal to the MedianBlurTemporal AVS plugin. It is slower with `maxr=1` and possibly with
    `maxr=2`, and significantly faster with `maxr > 2` Also it provides additional functions of possible threshold to limit max difference
    with source sample and some optional temporal IIR filtering.
    More info [here](https://github.com/Asd-g/AviSynth-vsTTempSmooth/issues/7#issuecomment-2083117936).<br>
...
- ythupd, uthupd, vthupd (only for `pmode=1`)<br>
    ...
    If y/u/vthupd > 0 - enable IIR-type filtering. It remembers previous "best" sample output and compares current "best" with memory value.
    If the difference below thupd - the memory sample used for output.
    More info [here](https://github.com/Asd-g/AviSynth-vsTTempSmooth/pull/8#issuecomment-1616112102).<br>
...
- ypnew, upnew, vpnew (only for `pmode=1`)<br>
    ...
    Allow to decrease the number of memory replacement operations and create more stable temporal output. Close to idea of penalty to
    update current best motion vector with new candidate with a bit better metric in MAnalyse from mvtools.
    Must be greater than 0.<br>
    Default: 0.
...
- radius_past, radius_future
    These parameters allow for asymmetric temporal smoothing by specifying the number of frames to look backward (`radius_past`) and forward
    (`radius_future`).<br>
    If specified, they override `maxr`.<br>
    Both parameters must be specified together.<br>
